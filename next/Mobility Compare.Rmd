...
```{r FOI_examples,echo=FALSE,message=FALSE,warning=FALSE, fig.path='Figs/',fig.width=10, fig.height=10}
     
mkFOI <- function(index)
{ 
  rv = list(foi_net_abs=NULL,foi_net_norm=NULL,foi_net_diff=NULL)
  
  I0=numeric(391)
  I0[which(lads_map$lad17nm==index)]=1
                     ...
                     foi_net <- tibble(from=lads_map$lad17nm[which(lads_map$lad17nm==index)],to=lads_map$lad17nm,weight=foi_gU$avg_inf_rate,data='Under 18') 
                     foi_net <- foi_net %>% bind_rows(tibble(from=lads_map$lad17nm[which(lads_map$lad17nm==index)],to=lads_map$lad17nm,weight=foi_g18_30$avg_inf_rate,data='18-30')) 
                     foi_net <- foi_net %>% bind_rows(tibble(from=lads_map$lad17nm[which(lads_map$lad17nm==index)],to=lads_map$lad17nm,weight=foi_g30_60$avg_inf_rate,data='30-60')) 
                     foi_net <- foi_net %>% bind_rows(tibble(from=lads_map$lad17nm[which(lads_map$lad17nm==index)],to=lads_map$lad17nm,weight=foi_g60_100$avg_inf_rate,data='60-100'))
                     
                     foi_net <- foi_net %>% filter(to!=index)
                     ...
                     lads_map_d1 = lads_map %>% filter(lad17nm!=index)
...
seed_loc = st_coordinates(st_centroid(lads_map %>% filter(lad17nm=='Waverley')))

p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         geom_sf(aes(fill=data),col='black',size=0.01) + 
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Waverley'),fill='black',size=0.01) 
#+                        annotate(geom='label',x=seed_loc[,1],y=seed_loc[,2],label='Waverley')
                         
p2=ggplot(lads_map %>% 
                                    inner_join(rv$foi_net_abs)) +
                       geom_sf(col='black',size=0.01,aes(fill=foi)) + 
                       scale_fill_distiller(palette ='Spectral',direction=-1,trans='log10') +
                       labs(fill='Net FoI') + ggtitle('B') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Waverley'),fill='black',size=0.01)
#+                         annotate(geom='label',x=seed_loc[,1],y=seed_loc[,2],label='Waverley')

p3=ggplot(lads_map) +
                         geom_sf(col='black',size=0.01,aes(fill=TotPop/AreaKm2)) + 
                         scale_fill_distiller(palette ='Spectral',direction=-1) +
                         labs(fill='Population\n Density (km-2)')  + ggtitle('C') 
#+                      geom_sf(data=lads_map %>% filter(lad17nm=='Waverley'),fill='black',size=0.01)
                     
p4=ggplot(lads_map) +
                         geom_sf(col='black',size=0.01,aes(fill=a60_100/TotPop)) + 
                         scale_fill_distiller(palette ='Spectral',direction=-1) +
                         labs(fill='Fraction 60-100') + ggtitle('D') 
#+                         geom_sf(data=lads_map %>% filter(lad17nm=='Waverley'),fill='black',size=0.01)
...
p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         ...
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Falkirk'),fill='black',size=0.01)
...
p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         ...
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Belfast'),fill='black',size=0.01)
...
p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         ...
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Newport'),fill='black',size=0.01)
...
p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         ...
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Cambridge'),fill='black',size=0.01)
...
p1=ggplot(lads_map %>% inner_join(rv$foi_net_norm %>%    mutate(lad17nm=to))) +
                         ...
                         scale_fill_brewer(palette ='Spectral',direction=-1) +
                         labs(fill='Age Group') + ggtitle('A') +
                         geom_sf(data=lads_map %>% filter(lad17nm=='Norwich'),fill='black',size=0.01)
