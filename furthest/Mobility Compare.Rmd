---
title: "Mobility Model Comparison (Furthest extent OD)"
...
total_flux <- as.matrix(read.csv('../flux/total_flux_furthest.csv')[,2:392])
under18_flux <- as.matrix(read.csv('../flux/under18_flux_furthest.csv')[,2:392])
education_flux <- as.matrix(read.csv('../flux/education_flux_furthest.csv')[,2:392])
employed_flux <- as.matrix(read.csv('../flux/employed_flux_furthest.csv')[,2:392])
neet_flux <- as.matrix(read.csv('../flux/neet_flux_furthest.csv')[,2:392])

a18_30_flux <- as.matrix(read.csv('../flux/age/a18_30_flux_furthest.csv')[,2:392])
a30_60_flux <- as.matrix(read.csv('../flux/age/a30_60_flux_furthest.csv')[,2:392])
a60_100_flux <- as.matrix(read.csv('../flux/age/a60_100_flux_furthest.csv')[,2:392])
...
```{r flux_by_lads ,echo=FALSE,message=FALSE,fig.path='Figs/',fig.width=7, fig.height=7*1.75}
...
df=tibble(census_p_move=zm$census_p_move,p_move=zm$p_move)
...
g1=ggplot(df,aes(x=census_p_move,y=p_move)) + geom_point() + 
  ...
  stat_poly_eq(formula = df$p_move ~ df$census_p_move,
               ...
               parse = TRUE) +
  labs(y=expression(italic(p[BBC])),x=expression(italic(p[C]))) + 
...
write.csv(age_df,file='../imputed_flux/moveProbByAgeLADS_furthest.csv')
```
...
emp_df <- emp_df %>% mutate(pred_m2r=predict(m1r,type='response'))
emp_df <- emp_df %>% mutate(group=factor(group,levels=c('Under 18', 'Education','Employed','NEET')))
...
write.csv(emp_df,file='../imputed_flux/moveProbByEmpLADS_furthest.csv')
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDOsubnational.csv',row.names = F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDOBBC.csv',row.names = F )
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDEsubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDEBBC.csv',row.names=F)
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDPsubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/CDPBBC.csv',row.names=F)
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/ERadsubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/ERadBBC.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/Stosubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/StoBBC.csv',row.names=F)
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/IOsubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/IOBBC.csv',row.names=F)
```
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/Impsubnational.csv',row.names=F)
...
write.csv(post_compare %>% group_by(.variable,data) %>% summarise(median=median(.value),lower=quantile(.value,0.025),upper=quantile(.value,0.975)) %>% arrange(data,.variable) %>% 
select(data,.variable,median,lower,upper),file = './csvtables/ImpBBC.csv',row.names=F)
...
ppCDOCE<-CDOmatflux(mobility_dat,fitCDO_CE,TRUE,FALSE,BBCmoves_by_LAD$census_p_move)
ppCDOCS<-CDOmatflux(mobility_dat,fitCDO_CS,TRUE,FALSE,BBCmoves_by_LAD$census_p_move)
ppCDOT<-CDOmatflux(mobility_dat,fitCDOT,TRUE,FALSE,BBCmoves_by_LAD$p_move)
...
ppCDOU<-CDOmatflux(mobility_dat,fitCDOU,TRUE,FALSE,unlist(age_df %>% filter(group=='Under 18') %>% select(pred_m1r)))
...
ppCDO18_30<-CDOmatflux(mobility_dat,fitCDO18_30,TRUE,FALSE,unlist(age_df %>% filter(group=='18-30') %>% select(pred_m1r)))
...
ppCDO30_60<-CDOmatflux(mobility_dat,fitCDO30_60,TRUE,FALSE,unlist(age_df %>% filter(group=='30-60') %>% select(pred_m1r)))

mobility_dat = list(N=lads_map$a60_100,
                    r = as.matrix(dist(st_coordinates(LADS_centroid %>% select(geometry)))),
                    flux = rep(1,length(lads_map$TotPop)))

ppCDO60_100<-CDOmatflux(mobility_dat,fitCDO60_100,TRUE,FALSE,unlist(age_df %>% filter(group=='60-100') %>% select(pred_m1r)))

imputed_flux<-as_tibble(melt(ppCDOCE$mean)) %>% mutate(data='E Census')
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDOT$mean)) %>% mutate(data='BBC Total'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDOU$mean)) %>% mutate(data='BBC Under 18'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO18_30$mean)) %>% mutate(data='BBC 18-30'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO30_60$mean)) %>% mutate(data='BBC 30-60'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO60_100$mean)) %>% mutate(data='BBC 60-100'))
...
write.table(tmp,file='../imputed_flux/ppCDOT_furthest.csv',row.names=T,col.names=T)

tmp <- ppCDOU$mean
colnames(tmp)<-lads_map$lad17cd
rownames(tmp)<-lads_map$lad17cd
write.table(tmp,file='../imputed_flux/ppCDOU_furthest.csv',row.names=T,col.names=T)

tmp <- ppCDO18_30$mean
colnames(tmp)<-lads_map$lad17cd
rownames(tmp)<-lads_map$lad17cd
write.table(tmp,file='../imputed_flux/ppCDO18_30_furthest.csv',row.names=T,col.names=T)

tmp <- ppCDO30_60$mean
colnames(tmp)<-lads_map$lad17cd
rownames(tmp)<-lads_map$lad17cd
write.table(tmp,file='../imputed_flux/ppCDO30_60_furthest.csv',row.names=T,col.names=T)

tmp <- ppCDO60_100$mean
colnames(tmp)<-lads_map$lad17cd
rownames(tmp)<-lads_map$lad17cd
write.table(tmp,file='../imputed_flux/ppCDO60_100_furthest.csv',row.names=T,col.names=T)
...
mkflux_mats <-function(theta, move_m, N)
{
  ...
  for(g in 1:Cg)
  {
    for(j in 1:P)
    {
      effectiveN[j] = effectiveN[j] + N[[g]][j]/(1+theta[g]*move_sums[[g]][j])
      for(i in 1:P)
      {
        effectiveN[j] = effectiveN[j] + theta[g]*move_m[[g]][i,j]*N[[g]][i]/(1+theta[g]*move_sums[[g]][i])
        fluxmat[[g]][j,i] = theta[g]*move_m[[g]][j,i]*flux_term[[g]][i]*flux_term[[g]][j]
      }
    }
