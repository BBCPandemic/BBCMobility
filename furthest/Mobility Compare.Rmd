...
ppCDOT_flux <- diag((1-BBCmoves_by_LAD$p_move)*lads_map$TotPop)+BBCmoves_by_LAD$p_move*lads_map$TotPop*ppCDOT_move$mean
...
ppCDO60_100_move<-CDOmovemat(mobility_dat,fitCDO60_100,TRUE)
ppCDO60_100_flux <- diag((1-p_move)*lads_map$TotPop)+p_move*lads_map$TotPop*ppCDO60_100_move$mean


imputed_flux<-as_tibble(melt(ppCDOCE_flux)) %>% mutate(data='E Census')
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDOT_flux)) %>% mutate(data='BBC Total'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDOU_flux)) %>% mutate(data='BBC Under 18'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO18_30_flux)) %>% mutate(data='BBC 18-30'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO30_60_flux)) %>% mutate(data='BBC 30-60'))
imputed_flux<-imputed_flux %>% bind_rows(as_tibble(melt(ppCDO60_100_flux)) %>% mutate(data='BBC 60-100'))
