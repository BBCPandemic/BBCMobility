...
ukoutline <- st_geometry(st_union(z[c(1:391),]))
...
f2 = ggplot(st_sf(z[1:406,]),aes(fill=p_move)) + 
  geom_sf(size=0.0) +
  scale_fill_gradient2(name ='Prop. Move',midpoint=mean(BBCmoves_by_LAD$p_move),low=muted('blue'),high=muted('red')) +
...
f1 = ggplot(st_sf(z[1:406,]),aes(fill=m_commuters/TotPop)) + geom_sf(size=0.0) +
  scale_fill_gradient2(name='Users (per capita)',midpoint=log10(mean(z$N/z$TotPop)),low=muted('blue'),high=muted('red'),trans='log10') +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) + 
